		...
		if (response_payloads) {
			try {
				...
				for (let i = 0; i < response_payloads.length; i++) {
					a.push(JSON.parse(response_payloads[0].toString('utf8')));
				}
				return a;
			} catch (error) {
				console.log("in query", response_payloads.toString().substring(7));
				try {
					JSON.parse(response_payloads.toString().substring(7));
					return JSON.parse(response_payloads.toString().substring(7));	
				} catch (error) {
					return response_payloads.toString().substring(7);	
				}
